...
def deform_conv(in_channels, out_channels, kernel_size, bias=True):
    return DeformConv2d(in_channels, out_channels, kernel_size, padding=(kernel_size // 2), bias=bias)
...
class knowledge_adaptation_UNet(nn.Module):
    def __init__(self):
        ...
        res2net101.load_state_dict(torch.load('./ImageNet.pth'))
...
def blockUNet(in_c, out_c, name, transposed=False, bn=False, relu=True, dropout=False):
    ...
    if not transposed:
        ...
        # block.add_module('%s_conv' % name, DeformConv2d(in_c, out_c, 4, 2, 1, bias=False))
    else:
